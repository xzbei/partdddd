...
#define TRANS_X_STD 5.0
#define TRANS_Y_STD 2.5
#define TRANS_S_STD 0.001
...
/*
  ...
  @return Returns an array of \a p particles sampled from around regions in
    \a regions
*/
particle* init_distribution( CvRect* regions, histogram** histos, int n, int p, int w, int h)
{
  particle* particles;
    gsl_rng* rng;
  int np;
  ...
  for( i = 0; i < n; i++ )
      ...
      for( j = 0; j < np; j++ )
	{
	  ...
	  //particles[k].y0 = particles[k].yp = particles[k].y = y;
        ...
        particles[k].x0 = particles[k].xp = particles[k].x = x + BB0 * rand() % w - (int)w/2;
        particles[k].y0 = particles[k].yp = particles[k].y = y + BB0 * rand() % h - (int)h/2;
