...
char* vid_file = "/Users/beixinzhu/Desktop/chairs.mp4";
...
int main( int argc, char** argv )
{
    ...
    //scanf ("%d",&firstframeindex);
    
    scanf ("%d",&firstframeindex);
    printf ("firstframe# %d\n",firstframeindex);
    
    for (int pp = 0; pp < firstframeindex; pp ++){
...
//                                          num_objects, num_particles , w , h);
        ...
        else
        {
            ...
            if (i == 1){}
                ...
                *regions = cvRect(particles[0].x, particles[0].y, particles[0].width,
                                  particles[0].height);
            }
            ...
            printf("Frame #%d, Initializing...\n",i);
            c =cvWaitKey(5);
            while (c!=13){c =cvWaitKey(5);}

            for (int k = 0;k < MAX_ITER_PER_FRAME; k++){
                frames[i] = cvClone( frame );
                ...
                //int lineWidth=2;
                //char str[7+5];
                //sprintf(str, "Frame #%d, Iter #%d" , i,k);
                //cvInitFont(&font,CV_FONT_HERSHEY_SIMPLEX, hScale,vScale,0,lineWidth,8);
                //cvPutText(frames[i],str,cvPoint(100,50),&font,CV_RGB(255,0,0));
